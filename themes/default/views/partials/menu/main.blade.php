...
<li class="nav-item @if($item->children->count() > 0) dropdown @endif">
    ...
    <ul class="dropdown-menu" aria-labelledby="{{ $item->name }}">
        ...
        @if($item->name == 'Admin')
        @role('admin')
        <li><a class="dropdown-item" href="@if($childItem->is_internal == 1){{ route($childItem->url) }}@else{{ url($childItem->url) }}@endif">{{ $childItem->name }}</a></li>
        @endrole
        @else
        <li><a class="dropdown-item" href="@if($childItem->is_internal == 1){{ route($childItem->url) }}@else{{ url($childItem->url) }}@endif" @if($childItem->name == 'Logout') onclick="event.preventDefault(); document.getElementById('logout-form').submit();" @else @endif>@if($childItem->name == 'Account Menu'){{ head(explode(' ', trim(Auth::user()->name))) }} <img src="{{ Auth::user()->profile->getProfileAvatar() }}" class="rounded-circle"  style="width: 20px;"> @else @action('submenu.nameBefore', $childItem){{ $childItem->name }}@action('submenu.nameAfter', $childItem)@endif</a></li>
        @endif
